...
//     return selectedSlot ? {
//       start_time: formatDate(dayjs(selectedSlot.start_time).toDate(), "hh:mm A"),
//       end_time: formatDate(dayjs(selectedSlot.end_time).toDate(), "hh:mm A"),
//     } :{
//       start_time: "",
//       end_time:"",
//     } 
...
//       // Show login prompt instead of opening dialog
...
//       const response = await axios.post("/api/bookings/book-appointment", {
//         ...formData,
//         slot_id: selectedSlot?.id,
//         service_id: selectedSlot?.service_id,
//       });
...
//           <Button className="w-full bg-accent hover:bg-accent/80 text-black py-6 rounded-lg font-semibold text-base transition-all transform active:scale-[0.98] shadow-sm h-auto">
//             Book My Slot
//           </Button>
//         </DialogTrigger>
...
//                 <Button
//                   type="submit"
//                   className="w-full sm:w-auto bg-pink text-black hover:bg-pink-700 font-semibold shadow-lg hover:shadow-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed"
//                   disabled={isLoading}
//                 >
...
export const BookingButton = ({ serviceName }: { serviceName: string }) => {
  ...
  // ✅ FIX: Update ALL form values when data changes
  useEffect(() => {
    if (userData) {
      formMethods.setValue("customer_name", userData.customer_name, {
        shouldValidate: true,
  ...
  // ✅ FIX: Add separate effect for slot data
  useEffect(() => {
    if (selectedSlot) {
  ...
  const submitForm = async (formData: FormData) => {
    try {
      ...
      // ✅ FIX: Explicitly ensure slot data is included
      const bookingData = {
